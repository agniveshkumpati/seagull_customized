...
//        GEN_DEBUG(1, "AGNI - C_ScenarioControl::add_actions() L_actionType = " << L_actionType);
	switch (L_actionType) {
...
int C_ScenarioControl::add_expression (char       *P_arg,
  ...
  GEN_DEBUG(1, "C_ScenarioControl::add_expression() start");
  int L_ret = 0 ;
  ...
  char   L_value[1500] ;		//AGNI - Changed from 100 to 1500
...
//   std::cerr << "buffer *****" << L_ptr << std::endl;
  ...
  *P_strExpr = NULL ;
  ...
  GEN_DEBUG(1, "C_ScenarioControl::add_expression() end");
  return (L_ret) ;
}
